    ...
    <aside className="fixed left-0 top-0 bottom-0 w-[224px] bg-[var(--surface)] border-r border-[var(--border)] flex flex-col z-50">
      ...
      <div className="relative mx-2.5 mt-3 mb-1">
        ...
        >
          ...
          <ChevronDown size={13} className={cn("text-[var(--muted)] flex-shrink-0 transition-transform duration-200", orgMenuOpen && "rotate-180")} />
        </button>
        ...
        {orgMenuOpen && (
          <>
            ...
            <div className="absolute top-full left-0 right-0 mt-1.5 bg-[var(--surface)] border border-[var(--border)] rounded-[var(--r-card)] shadow-card-hover z-20 overflow-hidden">
              <div className="px-3 py-2 border-b border-[var(--border)]">
                <p className="text-[10px] font-medium text-[var(--muted)] uppercase tracking-[0.6px]">Your organisations</p>
              </div>
              {allMemberships.map((m) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-[var(--muted)]">{roleLabels[m.role] ?? m.role}</p>
                  </div>
              ...
              <div className="border-t border-[var(--border)]">
                <button
                  ...
                  className="w-full flex items-center gap-2 px-3 py-2.5 text-[12px] text-[var(--muted)] hover:text-[var(--text)] hover:bg-[var(--bg)] transition-colors"
                >
      ...
      <nav className="flex-1 overflow-y-auto py-1">
        {navItems.map((group) => (
          <div key={group.section}>
            <p className="px-4 pt-4 pb-1.5 text-[10px] font-medium text-[var(--muted)] uppercase tracking-[0.8px]">
              {group.section}
            </p>
            {group.items.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  {!!item.badge && (
                    <span className="bg-[var(--primary)] text-white text-[10px] font-semibold px-1.5 py-0.5 rounded-full min-w-[18px] text-center leading-tight">
                      {item.badge}
                    </span>
                  )}
                </Link>
