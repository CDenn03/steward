...
const COLORS = ["#4B6650", "#A6672E", "#79766B", "#B94A3F", "#1E2A24", "#6B9972"];
    ...
    <>
      <PageHeader title="Analytics" subtitle="Budget variance, income trends, and department utilisation">
        ...
        <select
          ...
          className="px-3 py-1.5 text-[13px] bg-(--surface) border border-(--border) rounded-(--r-input) outline-none focus:border-(--primary) text-(--text) transition-colors"
        >
          {years.map((y) => (
          ...
          <div className="p-5">
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={departmentSpend} margin={{ top: 0, right: 0, bottom: 0, left: 0 }}>
                ...
                <YAxis tick={{ fontSize: 11, fill: "#79766B" }} axisLine={false} tickLine={false} tickFormatter={(v: number) => `${v / 1000}k`} />
                <Tooltip content={<CustomTooltip />} />
                <Bar dataKey="allocated" name="Allocated" fill="#E8E3D6" radius={[3, 3, 0, 0]} />
                <Bar dataKey="spent" name="Spent" fill="#4B6650" radius={[3, 3, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </Card>
          ...
          <div className="p-5">
            <ResponsiveContainer width="100%" height={220}>
              <LineChart data={monthlyIncome} margin={{ top: 0, right: 10, bottom: 0, left: 0 }}>
                ...
                <YAxis tick={{ fontSize: 11, fill: "#79766B" }} axisLine={false} tickLine={false} tickFormatter={(v: number) => `${v / 1000}k`} />
                <Tooltip content={<CustomTooltip />} />
                <Line type="monotone" dataKey="offerings" name="Offerings" stroke="#4B6650" strokeWidth={2} dot={false} />
                <Line type="monotone" dataKey="donations" name="Donations" stroke="#A6672E" strokeWidth={2} dot={false} strokeDasharray="4 2" />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </Card>
        ...
        <Card>
          ...
          <div className="px-5 pb-5 space-y-2">
            {pieData.map((department, i) => (
              <div key={department.name} className="flex items-center gap-2 text-[13px]">
            ...
            <thead>
              <tr className="border-b border-(--border) bg-(--bg)">
                {["Department", "Allocated", "Spent", "Variance", "Util."].map((h) => (
                  <th key={h} className="text-left text-[12px] font-medium text-(--muted) uppercase tracking-[0.5px] px-4 py-2.5">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {departmentSpend.map((department) => {
                ...
                return (
                  <tr key={department.id} className="border-b border-(--border) last:border-0 hover:bg-(--bg) transition-colors">
                    <td className="px-4 py-3 text-[14px] font-medium">{department.name}</td>
                    <td className="px-4 py-3 text-[14px] font-mono text-(--muted)">{formatCurrency(department.allocated, currency, true)}</td>
                    <td className="px-4 py-3 text-[14px] font-mono">{department.spent > 0 ? formatCurrency(department.spent, currency, true) : "-"}</td>
                    <td className={`px-4 py-3 text-[14px] font-mono ${department.variance >= 0 ? "text-success" : "text-danger"}`}>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[12px] text-(--muted) w-8 text-right">{percent}%</span>
                      </div>
                    </td>
                  </tr>
                );
