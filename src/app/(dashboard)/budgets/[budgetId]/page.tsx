        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-(--surface) border border-(--border) rounded-(--r-card) px-4 py-3.5">
              <p className="text-[12px] font-medium text-(--muted) uppercase tracking-[0.5px] mb-2">Total Budget</p>
              <p className="text-[20px] font-semibold tracking-tight font-mono">{formatCurrency(totalAmount)}</p>
            </div>
            <div className="bg-(--surface) border border-(--border) rounded-(--r-card) px-4 py-3.5">
              <p className="text-[12px] font-medium text-(--muted) uppercase tracking-[0.5px] mb-2">Line Items</p>
              <p className="text-[20px] font-semibold tracking-tight">{items.length}</p>
            </div>
            <div className="bg-(--surface) border border-(--border) rounded-(--r-card) px-4 py-3.5">
              <p className="text-[12px] font-medium text-(--muted) uppercase tracking-[0.5px] mb-2">Disbursed</p>
              <p className="text-[20px] font-semibold tracking-tight font-mono">{formatCurrency(totalDisbursed)}</p>
            </div>
            <div className="bg-(--surface) border border-(--border) rounded-(--r-card) px-4 py-3.5">
              <p className="text-[12px] font-medium text-(--muted) uppercase tracking-[0.5px] mb-2">Claimed</p>
          ...
          <Card>
            <CardHeader>
              <CardTitle>
                ...
                <p className="text-[13px] text-(--muted)">{items.length} items</p>
              </CardTitle>
            </CardHeader>
              ...
              <thead>
                <tr className="border-b border-(--border) bg-(--bg)">
                  {["Description", "Category", "Qty", "Unit Cost", "Total"].map((h) => (
                    <th key={h} className="text-left text-[12px] font-medium text-(--muted) uppercase tracking-[0.5px] px-4 py-2.5 last:text-right">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {items.map((item: BudgetItemRow) => (
                  <tr key={item.id} className="border-b border-(--border) last:border-0 hover:bg-(--bg) transition-colors">
                    <td className="px-4 py-3 text-[14px] font-medium">{item.description}</td>
                    <td className="px-4 py-3 text-[13px] text-(--muted)">{item.category?.name ?? "—"}</td>
                    <td className="px-4 py-3 text-[14px] font-mono text-(--muted)">{item.quantity}</td>
                    <td className="px-4 py-3 text-[14px] font-mono text-(--muted)">{formatCurrency(item.unitCost)}</td>
                    <td className="px-4 py-3 text-[14px] font-mono font-medium text-right">{formatCurrency(item.totalCost)}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr className="border-t-2 border-(--border) bg-(--bg)">
                  <td colSpan={4} className="px-4 py-3 text-[14px] font-semibold">Total</td>
          ...
          <Card>
            <CardHeader>
              <CardTitle>
                ...
                <p className="text-[13px] text-(--muted)">{disbursements.length} records</p>
              </CardTitle>
            ...
            <CardBody className="p-0 divide-y divide-(--border)">
              {disbursements.length === 0 ? (
                <div className="px-4 py-8 text-center text-[14px] text-(--muted)">No disbursements recorded</div>
              ) : (
                disbursements.map((disbursement: DisbursementRow) => (
                  <div key={disbursement.id} className="flex items-center justify-between gap-4 px-4 py-3">
                    <div className="min-w-0">
                      <p className="text-[14px] font-medium truncate">{disbursement.description}</p>
                      <p className="text-[12px] text-(--muted)">{formatRelative(disbursement.createdAt)}</p>
                    </div>
                    <div className="flex items-center gap-3">
                      <span className="font-mono text-[14px] font-medium">{formatCurrency(disbursement.totalAmount)}</span>
                      <span className="text-[11px] font-medium bg-[var(--primary-light)] text-(--primary) px-1.5 py-0.5 rounded capitalize">
          ...
          <Card>
            <CardHeader>
              <CardTitle>
                ...
                <p className="text-[13px] text-(--muted)">{expenditures.length} reports</p>
              </CardTitle>
            </CardHeader>
            <CardBody className="p-0 divide-y divide-(--border)">
              {expenditures.length === 0 ? (
                <div className="px-4 py-8 text-center text-[14px] text-(--muted)">No expenditure reports submitted</div>
              ) : (
                expenditures.map((report: ExpenditureRow) => (
                  <div key={report.id} className="flex items-center justify-between gap-4 px-4 py-3">
                    <div className="min-w-0">
                      <p className="text-[14px] font-medium truncate">{report.title}</p>
                    ...
                    <div className="flex items-center gap-3">
                      <span className="font-mono text-[14px] font-medium">{formatCurrency(report.totalApproved ?? report.totalClaimed)}</span>
          ...
          <Card>
            <CardHeader>
              <CardTitle>
                ...
                <p className="text-[13px] text-(--muted)">{attachments.length} files</p>
              </CardTitle>
            </CardHeader>
          ...
          <Card>
            <CardHeader>
              <CardTitle>
                ...
                <p className="text-[13px] text-(--muted)">{revisions.length} revisions</p>
              </CardTitle>
            </CardHeader>
            <CardBody className="p-0">
            ...
            <CardBody className="space-y-4">
              {[
                ...
                <div key={s.step} className="flex items-center gap-3">
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 text-[12px] font-semibold ${
                    s.done ? "bg-success text-white" : s.active ? "bg-(--primary) text-white" : "bg-(--border) text-(--muted)"
                  }`}>{s.done ? "✓" : i + 1}</div>
                  <div>
                    <p className={`text-[14px] ${s.done || s.active ? "font-medium" : "text-(--muted)"}`}>{s.step}</p>
                    {s.active && <p className="text-[12px] text-(--primary)">In progress</p>}
                    {s.done && <p className="text-[12px] text-success">Completed</p>}
                  </div>
                </div>
              ))}
            </CardBody>
            ...
            <CardBody className="space-y-3">
              {[
                ...
                <div key={d.label} className="flex justify-between gap-4">
                  ...
                  <span className="text-[13px] font-medium text-right">{d.value}</span>
                </div>
              ))}
            </CardBody>
          </Card>
