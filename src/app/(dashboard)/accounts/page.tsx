          ...
          <Card key={account.id} className="hover:shadow-card-hover transition-shadow">
            <CardBody>
              <div className="flex items-start justify-between mb-4">
                <div className="w-10 h-10 rounded-[10px] bg-[var(--primary-light)] flex items-center justify-center text-[11px] font-semibold">
                  {account.type}
                </div>
                <span className={`text-[11px] font-medium px-2 py-0.5 rounded-md ${account.isActive ? "bg-success-bg text-success" : "bg-draft-bg text-draft"}`}>
                  {account.isActive ? "Active" : "Inactive"}
                </span>
              </div>
              <p className="text-[14px] font-medium mb-0.5">{account.name}</p>
              <p className="text-[12px] text-(--muted) mb-4">{account.provider ?? "Provider"} · {account.accountNumber ?? "No account number"}</p>
              <div className="border-t border-(--border) pt-3">
                <p className="text-[12px] text-(--muted) uppercase tracking-[0.5px] font-medium mb-1">Balance</p>
      ...
      <Card>
        <CardHeader>
          <CardTitle>
            ...
            <p className="text-[13px] text-(--muted)">Across all accounts</p>
          </CardTitle>
