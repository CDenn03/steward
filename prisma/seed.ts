 ...
 * Requires:   pnpm db:generate  (then pnpm db:migrate) first
 */

// Dynamic import so this file compiles even before `prisma generate` has run.
// After generate, the types will be resolved correctly.
async function main() {
  ...
  try {
    ...
    const users = await Promise.all([
      prisma.user.upsert({ where: { email: "james@gracecommunity.org" },   update: {}, create: { name: "James Mwangi",    email: "james@gracecommunity.org" } }),
      prisma.user.upsert({ where: { email: "sarah@gracecommunity.org" },   update: {}, create: { name: "Sarah Kamau",     email: "sarah@gracecommunity.org" } }),
      prisma.user.upsert({ where: { email: "peter@gracecommunity.org" },   update: {}, create: { name: "Peter Odhiambo", email: "peter@gracecommunity.org" } }),
      prisma.user.upsert({ where: { email: "chair@gracecommunity.org" },   update: {}, create: { name: "Daniel Njoroge", email: "chair@gracecommunity.org" } }),
      prisma.user.upsert({ where: { email: "grace@gracecommunity.org" },   update: {}, create: { name: "Grace Wanjiku",  email: "grace@gracecommunity.org" } }),
    ]);
    ...
    await prisma.membership.upsert({
      where: { userId_organizationId: { userId: users[0].id, organizationId: org.id } },
      update: {},
      create: { userId: users[0].id, organizationId: org.id, role: "FINANCE" },
    });
    await prisma.membership.upsert({
      where: { userId_organizationId: { userId: users[3].id, organizationId: org.id } },
      update: {},
      create: { userId: users[3].id, organizationId: org.id, role: "CHAIRPERSON" },
    });
    await prisma.membership.upsert({
      where: { userId_organizationId: { userId: users[1].id, organizationId: org.id } },
      update: {},
      create: { userId: users[1].id, organizationId: org.id, role: "DEPARTMENT_HEAD", departmentId: departments[4].id },
    });
    await prisma.membership.upsert({
      where: { userId_organizationId: { userId: users[2].id, organizationId: org.id } },
      update: {},
      create: { userId: users[2].id, organizationId: org.id, role: "DEPARTMENT_HEAD", departmentId: departments[2].id },
    });
    await prisma.membership.upsert({
      where: { userId_organizationId: { userId: users[4].id, organizationId: org.id } },
      update: {},
      create: { userId: users[4].id, organizationId: org.id, role: "FINANCE" },
    });
    ...
    for (const [i, inc] of incomeData.entries()) {
      await prisma.income.upsert({
        ...
        create: {
          ...inc,
          organizationId: org.id,
          recordedById: users[i % 2 === 0 ? 0 : 4].id,
          receivedAt: new Date(Date.now() - i * 86400000 * 3),
        },
      });
    }
